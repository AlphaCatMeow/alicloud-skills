...
| network/esa | alicloud-network-esa | Manage Alibaba Cloud ESA — deploy HTML/static sites via Pages, manage Edge Routines (ER) for serverless edge functions, use Edge KV for distributed key-value storage, and handle site management, DNS records, cache rules via OpenAPI/SDK. Use when working with ESA, edge deployment, edge functions, Pages, ER, KV storage, DNS, cache, or site configuration. | `skills/network/esa/alicloud-network-esa` |
