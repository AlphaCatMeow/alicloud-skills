---
name: alicloud-ai-video-liveportrait
...
```bash
mkdir -p output/alicloud-ai-video-liveportrait
python -m py_compile skills/ai/video/alicloud-ai-video-liveportrait/scripts/prepare_liveportrait_request.py && echo "py_compile_ok" > output/alicloud-ai-video-liveportrait/validate.txt
```

Pass criteria: command exits 0 and `output/alicloud-ai-video-liveportrait/validate.txt` is generated.

## Output And Evidence

- Save normalized request payloads, template choice, and task polling snapshots under `output/alicloud-ai-video-liveportrait/`.
...
```bash
python skills/ai/video/alicloud-ai-video-liveportrait/scripts/prepare_liveportrait_request.py \
...
- Default output: `output/alicloud-ai-video-liveportrait/request.json`
